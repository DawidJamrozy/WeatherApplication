...
fun AddCurrentLocation(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.screen_my_locations_add_current_location),
            color = LocalCustomColorsPalette.current.text,
            fontSize = 16.sp
        )

        Text(
            text = stringResource(R.string.screen_my_locations_add_your_location_details),
        ...
        Button(
            onClick = {
                if (permissionState.status.isGranted) {
                    onEvent(MyLocationsScreenViewEvent.OnLocationPermissionGranted)
                } else {
            ...
            if (isLoading) {
                ...
                Text(
                    text = stringResource(R.string.screen_my_locations_find_me),
                    color = LocalCustomColorsPalette.current.textReverse,
                    fontSize = 16.sp
                )
            }
